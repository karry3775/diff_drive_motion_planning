"""ROS2 node: records odom data, publishes robot trail, saves plots."""
...
class RecorderNode(Node):
    def __init__(self):
        ...
        self.robot_x, self.robot_y, self.robot_theta, self.robot_time = [], [], [], []
        ...
        self.create_subscription(Path, '/smoothed_path', self._on_path, 10)
        self.create_subscription(Path, '/raw_waypoints', self._on_waypoints, 10)
    ...
    def _on_odom(self, msg):
        ...
        if dist < self.goal_tol and len(self.robot_x) > 50:
            ...
            self.get_logger().info(f'Goal reached! Recorded {len(self.robot_x)} samples')
            self._save_plots()
    ...
    def _save_plots(self):
        ...
        if len(rt) > 1:
            dt = np.diff(rt)
            dt[dt < 1e-9] = 1e-9
            actual_vel = np.sqrt(np.diff(rx)**2 + np.diff(ry)**2) / dt
            ax.plot(rt[1:], actual_vel, 'g-', label='Actual', alpha=0.7)
        if self.traj_vel is not None:
            traj_t = np.linspace(0, rt[-1], len(self.traj_vel))
            ax.plot(traj_t, self.traj_vel, 'b--', label='Reference', alpha=0.7)
...
def main(args=None):
    ...
    try:
        ...
        if not node.goal_reached and len(node.robot_x) > 10:
            node.get_logger().info('Shutdown — saving plots')
            node._save_plots()
        node.destroy_node()
        rclpy.shutdown()
